...
## Cleartest DB Credentials
DataGrip normally stores DB passwords in the system ("Keychain"). On macOS this is Keychain Access. On other systems it will be different. The credentials are stored and identified by the DataSource's GUID. Identifying each individual system's or OS's Keychain and being able to interact with it would be a nightmare, and even moreso would be identifying how to properly name these credentials so DataGrip can see and understanad them.

Instead, the DDEV DB credentials are stored in plaintext in the JDBC URL within the `dataSources.xml` file. By default, DDEV uses `root:root` as the username and password. Since DDEV is meant to be a local development environmemnt, this is typically fine, and commiting these credentials isn't a huge issue. However, if you are using different credentials, do NOT commit `.ddev/datagrip` to Git! **They WILL leak**.