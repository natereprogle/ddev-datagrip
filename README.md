...
**You are not required to utilize the LivePlugin plugin!** The Add-On will operate 100% fine without it. However, doing so will improve your experience with this Add-On greatly. You will be asked if you want to install it when installing the Add-On (handled via the native `installPlugins` argument in the datagrip CLI launcher), but you can always decline and either manually refresh whenever needed, or install the plugin manually at a later time.
